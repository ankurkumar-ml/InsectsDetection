...
from config import bees_weight_path
...
@torch.no_grad()
def run(weights=bees_weight_path,  # model.pt path(s)
    ...
    # Directories
    save_dir = save_to_img_path #"D:\\Other\\Bees\\images\\boxes"
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img or save_crop or view_img:  # Add bbox to image
                        ...
                        if label == 'B' and conf.item() > conf_thresh:
                            bees_count += 1


                dataframe.loc[len(dataframe.index)] = [filename, bees_count]
            ...
            im0 = cv2.putText(im0, 'Total Honey Bees: {}'.format(bees_count), org, font,
...
def bees_prediction(filename, src_img_path, dst_img_path):
    df = pd.DataFrame(columns=["filename", "bees_count"])
    run(source=filename, dataframe=df, src_img_path=src_img_path, save_to_img_path=dst_img_path)

    bees_count = list(df['bees_count'])[0]

    return filename, bees_count

def multiple_files():
    data_path = "D:\\Other\\Bees\\images\\raw"
    ...
    df.to_excel(r"D:\\Other\\Bees\\images\\bees_count_result.xlsx")

def process_cgt(filename):
    df = pd.DataFrame(columns=["Filename", "Detected Rows", "Stamp_path", "Stamp_confidence", "row_count_confidence"])
    run(source=filename, dataframe=df)
    print(df)